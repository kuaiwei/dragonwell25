...
void LIRItem::load_byte_item() { load_item(); }
...
LIR_Opr LIRGenerator::divInOpr() {
  Unimplemented();
  return LIR_OprFact::illegalOpr;
}
LIR_Opr LIRGenerator::divOutOpr() {
  Unimplemented();
  return LIR_OprFact::illegalOpr;
}
LIR_Opr LIRGenerator::remOutOpr() {
  Unimplemented();
  return LIR_OprFact::illegalOpr;
}
LIR_Opr LIRGenerator::shiftCountOpr() {
  Unimplemented();
  return LIR_OprFact::illegalOpr;
}
...
LIR_Opr LIRGenerator::result_register_for(ValueType *type, bool callee) {
  ...
  case intTag:
    ...
    break;
  case objectTag:
    opr = FrameMap::r0_oop_opr;
    break;
  case longTag:
    opr = FrameMap::long0_opr;
    break;
  case floatTag:
    opr = FrameMap::fpu0_float_opr;
    break;
  case doubleTag:
    opr = FrameMap::fpu0_double_opr;
    break;
...
bool LIRGenerator::can_inline_as_constant(Value v) const {
  // FIXME: Just a guess
  if (v->type()->as_IntConstant() != nullptr) {
    return Assembler::operand_valid_for_add_sub_immediate(
        v->type()->as_IntConstant()->value());
  } else if (v->type()->as_LongConstant() != nullptr) {
    return v->type()->as_LongConstant()->value() == 0L;
  } else if (v->type()->as_ObjectConstant() != nullptr) {
    return v->type()->as_ObjectConstant()->value()->is_null_object();
...
LIR_Address *LIRGenerator::generate_address(LIR_Opr base, LIR_Opr index,
  ...
  if (index->is_register()) {
    ...
    if (shift > 0) {
      // Use long register to avoid overflow when shifting large index values
      // left.
      LIR_Opr tmp = new_register(T_LONG);
  ...
  } else if (large_disp != 0 &&
             !Address::offset_ok_for_immed(large_disp, shift)) {
    // index is illegal so replace it with the displacement loaded into a
    // register
  ...
  if (large_disp == 0 && index->is_register()) {
    ...
    assert(Address::offset_ok_for_immed(large_disp, shift),
           "failed for large_disp: " INTPTR_FORMAT " and shift %d", large_disp,
           shift);
...
void LIRGenerator::store_stack_parameter(LIR_Opr item,
  ...
  __ store(item,
           new LIR_Address(FrameMap::sp_opr, in_bytes(offset_from_sp), type));
}
...
void LIRGenerator::do_ArithmeticOp_Long(ArithmeticOp *x) {
  ...
  if (x->op() == Bytecodes::_ldiv || x->op() == Bytecodes::_lrem) {
    ...
    assert(x->op() == Bytecodes::_lmul || x->op() == Bytecodes::_ladd ||
    ...
    if (!right.is_register()) {
      if (x->op() == Bytecodes::_lmul || !right.is_constant() ||
          !Assembler::operand_valid_for_add_sub_immediate(
              right.get_jlong_constant())) {
        right.load_item();
      } else { // add, sub
        assert(x->op() == Bytecodes::_ladd || x->op() == Bytecodes::_lsub,
               "expect ladd or lsub");
...
void LIRGenerator::do_ArithmeticOp_Int(ArithmeticOp *x) {
  ...
  if (x->op() == Bytecodes::_idiv || x->op() == Bytecodes::_irem) {
    ...
    if (x->op() == Bytecodes::_irem) {
      __ irem(left_arg->result(), right_arg->result(), x->operand(), ill,
              nullptr);
    } else if (x->op() == Bytecodes::_idiv) {
      __ idiv(left_arg->result(), right_arg->result(), x->operand(), ill,
              nullptr);
    }

  } else if (x->op() == Bytecodes::_iadd || x->op() == Bytecodes::_isub) {
    if (right.is_constant() && Assembler::operand_valid_for_add_sub_immediate(
                                   right.get_jint_constant())) {
      right.load_nonconstant();
    } else {
      right.load_item();
    }
    ...
    arithmetic_op_int(x->op(), x->operand(), left_arg->result(),
                      right_arg->result(), LIR_OprFact::illegalOpr);
  } else {
    assert(x->op() == Bytecodes::_imul, "expect imul");
    if (right.is_constant()) {
      jint c = right.get_jint_constant();
      if (c > 0 && c < max_jint &&
          (is_power_of_2(c) || is_power_of_2(c - 1) || is_power_of_2(c + 1))) {
        right_arg->dont_load_item();
      } else {
    ...
    rlock_result(x);
    arithmetic_op_int(x->op(), x->operand(), left_arg->result(),
                      right_arg->result(), new_register(T_INT));
  }
}
...
void LIRGenerator::do_LogicOp(LogicOp *x) {
  ...
  rlock_result(x);
  if (right.is_constant() && ((right.type()->tag() == intTag &&
                               Assembler::operand_valid_for_logical_immediate(
                                   true, right.get_jint_constant())) ||
                              (right.type()->tag() == longTag &&
                               Assembler::operand_valid_for_logical_immediate(
                                   false, right.get_jlong_constant())))) {
    right.dont_load_item();
  } else {
    right.load_item();
  }
  ...
  case Bytecodes::_land:
    __ logical_and(left.result(), right.result(), x->operand());
    break;
  case Bytecodes::_ior:
  case Bytecodes::_lor:
    __ logical_or(left.result(), right.result(), x->operand());
    break;
  case Bytecodes::_ixor:
  case Bytecodes::_lxor:
    __ logical_xor(left.result(), right.result(), x->operand());
    break;
  default:
    Unimplemented();
  }
...
void LIRGenerator::do_NewInstance(NewInstance *x) {
  ...
  new_instance(reg, x->klass(), x->is_unresolved(),
               ...
               FrameMap::r10_oop_opr, FrameMap::r11_oop_opr,
               FrameMap::r4_oop_opr, LIR_OprFact::illegalOpr,
...
void LIRGenerator::do_If(If *x) {
  ...
  if (tag == longTag) {
    if (yin->is_constant() && Assembler::operand_valid_for_add_sub_immediate(
                                  yin->get_jlong_constant())) {
      yin->dont_load_item();
    } else {
      yin->load_item();
    }
